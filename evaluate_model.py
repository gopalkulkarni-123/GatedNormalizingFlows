...
import pickle
#from train_ae import model_name

filename = "/bigdata/hplsim/aipp/Gopal/data_npy/electron_data_processed.pkl"
...
def define_options_parser():
    ...
    parser.add_argument('--experiment_path', type=str, default = "/home/damoda95/Upload/results/airplane_gen_model", help='Path to experiment.')
    #parser.add_argument('--modelname', type=str, default = 'airplane_gen_model', help='Model name (without ending).')
    parser.add_argument('--part', default = 'test', help='Part of dataset (train / val / test).')
    parser.add_argument('--cloud_size', default = 5000, type=int, help='Number of input points.')
    parser.add_argument('--sampled_cloud_size', default = 5000, type=int, help='Number of sampled points.')
...
def eval_main(model_name, generated_file, cond):

    parser = define_options_parser()
    args = parser.parse_args()
    with io.open(os.path.join('/home/damoda95/Upload_v2/go_with_the_flows/configs/config_generative_modeling_FEL.yaml'), 'r') as stream:
    ...
    path2checkpoint = os.path.join(config['experiment_path'], "{}.pkl".format(model_name)) #Get it from train
    ...
    if config['util_mode'] == 'autoencoding' or config['util_mode'] == 'reconstruction':
        res = evaluate(eval_iterator, model, generated_file, criterion, **config)
        print(res)
        ...
        for i in range(args.reps):
            res_per = evaluate(eval_iterator, model, generated_file, criterion, **config)
        ...
        print(res)
        """
    #wandb.init(project="electron_data", name="FEL_v5_results")
    evaluate(eval_iterator, model, generated_file, criterion, cond, **config)